...
public sealed class Repository<TDb>(TDb dbContext) : IRepository<TDb> where TDb : DbContext
{
    ...
    public async Task<bool> AnyAsync<T>(ISpecification<T> spec, CancellationToken cancellationToken = default) where T : BaseEntity =>
        await new SpecificationEvaluator().GetQuery(DbContext.Set<T>().AsQueryable(), spec).AnyAsync(cancellationToken);
