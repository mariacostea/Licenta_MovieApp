...
public interface IRepository<out TDb> where TDb : DbContext
{
    ...
    public interface IRepository<TDb> :
        IRepositoryBase<AggregateRoot>,
        IReadRepositoryBase<AggregateRoot>    
        where TDb : DbContext
    { }
    
    Task<T?> GetByIdAsync<T>(Guid id, CancellationToken ct = default) where T : BaseEntity;

    Task<bool> AnyAsync<T>(ISpecification<T> spec, CancellationToken ct = default) where T : BaseEntity;
